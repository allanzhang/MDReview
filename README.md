...
- **代码块**：语言栏 + 行号 + 右上角 Copy 按钮；长行自适应换行；MDReview 统一调色板高亮（蓝=关键字 / 绿=字符串 / 琥珀=数字 / 紫=函数，浅深同色相变体）；**点语言栏整体折叠**，折叠态显示行数（`▸ 语言 · N lines`）
...
- **字号调节**：正文基准字号读系统 body 文本样式（放大系统字号，正文/标题/代码等比缩放）；右上角 `A−` / `A+` 浮胶囊可再缩放 0.8×~1.5×（步进 15%，改 CSS 变量即时生效，不重载不丢滚动；不跨启动持久）

### 导航
- **大纲双向同步**：侧栏 Outline 点选平滑跳转；正文滚动时反向高亮当前章节，大纲跟随滚动保持激活条目可视；点击后程序化滚动不会抢走选中
- **标题树层级**：H1 与 H2+ 两级缩进，层级主要由字号 + 字重 + 文字色区分（H1 13pt semibold / H2+ 渐淡递减），一眼可辨；有子标题的节点带 ▶/▼ 三角可折叠/展开子树；标题强制单行、超长截断为 …，hover tooltip 展示全文
...
- **阅读进度条**：内容区顶部 2pt 中性灰进度条（浅色冷灰 / 深色中性灰，不刺眼）
- **大纲思写加载骨架**：打开大文档时预扫描标题逐条淡入 + 顶部 2pt 扫描细线，替代白屏等待；无转圈、无伪进度，渲染完成整体淡出
...
- 关键机制：
  ...
  - **消息通道**：页面脚本 `postMessage` 回推大纲 / 当前章节 / 阅读进度（0~1）/ 链接点击到 Swift
